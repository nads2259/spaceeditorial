<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.subcategories.edit', $subcategory) }}" class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">{{ __('Edit') }}</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="p-6">
                    <dl class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $subcategory->category->name ?? '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('Slug') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $subcategory->slug }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('Sort Order') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $subcategory->sort_order }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">{{ __('Active') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $subcategory->is_active ? __('Yes') : __('No') }}</dd>
                        </div>
                        <div class="sm:col-span-2">
                            <dt class="text-sm font-medium text-gray-500">{{ __('Description') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $subcategory->description ?? '—' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <ul class="space-y-3">
                        @forelse ($subcategory->posts->take(10) as $post)
